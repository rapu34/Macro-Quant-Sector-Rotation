...
"""
    ...
    # -------------------------------------------------------------------------
    # Benchmark comparison (Portfolio vs SPY) for dashboard
    # -------------------------------------------------------------------------
    PPY = 252
    ...
    bench_daily = pd.DataFrame({
        "date": merged.index,
        "r_port": merged["r_p"].values,
        "r_spy": merged["r_spy"].values,
        "cum_port": cum_p.values,
        "cum_spy": cum_s.values,
    })
    bench_daily.to_csv(EXP_OUT / "benchmark_daily.csv", index=False)
    ...
    bench_metrics = {
        ...
        "downside_deviation": dd_vol,
        "ann_ret_portfolio": ann_ret_p,
        "ann_ret_spy": ann_ret_s,
    }
    ...
    # -------------------------------------------------------------------------
    # Regime Performance Analysis (Core / Elevated / Crisis)
    # -------------------------------------------------------------------------
    ...
    if regime_path.exists():
        try:
            ...
            if "P_Crisis" in regime_df.columns:
                regime_df["regime"] = np.where(regime_df["P_Crisis"] >= 0.6, "Crisis",
                                                np.where(regime_df["P_Crisis"] >= 0.4, "Elevated", "Core"))
            ...
            print(f"[WARN] Regime analysis skipped: {e}")
    ...
    alert_rows = []
    crisis_events = [
        ("2020-03-09", "SPY crash (COVID)"),
        ("2020-03-12", "SPY crash (COVID)"),
        ("2020-03-16", "SPY crash (COVID)"),
        ("2022-06-13", "VIX spike (Rate shock)"),
        ("2025-04-04", "Volatility shock (Tariff)"),
    ]
    for d, label in crisis_events:
        if pd.Timestamp(d) in merged.index:
            alert_rows.append({"Date": d, "Alert": label})
    if worst5_df is not None and len(worst5_df) > 0:
        for idx in worst5_df.index[:5]:
            d = str(idx.date()) if hasattr(idx, "date") else str(idx)[:10]
            dv = worst5_df.loc[idx, "delta_vix"] * 100 if "delta_vix" in worst5_df.columns else 0
            lab = "VIX spike" if dv > 30 else "SPY crash"
            if not any(r["Date"] == d for r in alert_rows):
                alert_rows.append({"Date": d, "Alert": lab})
    if alert_rows:
        pd.DataFrame(alert_rows).sort_values("Date", ascending=False).drop_duplicates("Date").head(20).to_csv(
            EXP_OUT / "alert_history.csv", index=False
        )
